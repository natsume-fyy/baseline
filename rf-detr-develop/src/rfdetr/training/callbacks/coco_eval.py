...
class COCOEvalCallback(Callback):
    ...
    def _compute_and_log(self, trainer: Any, pl_module: Any, split: str, *, metric: Any | None = None) -> None:
        ...
        pl_module.log(
            f"{split}/mAP_small", metrics[f"{pfx}map_small"], prog_bar=True, logger=True, on_step=False, on_epoch=True
        )
        pl_module.log(
            f"{split}/mAP_medium", metrics[f"{pfx}map_medium"], prog_bar=True, logger=True, on_step=False, on_epoch=True
        )
        pl_module.log(
            f"{split}/mAP_large", metrics[f"{pfx}map_large"], prog_bar=True, logger=True, on_step=False, on_epoch=True
        )
